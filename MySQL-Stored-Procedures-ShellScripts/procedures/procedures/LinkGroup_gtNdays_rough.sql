delimiter |
DROP PROCEDURE IF EXISTS groupLink_gt1Day;
CREATE PROCEDURE groupLink_gt1Day(IN startTime VARCHAR(20), IN endTime VARCHAR(20))
BEGIN
...
DROP TEMPORARY TABLE IF EXISTS TRAP_DOWN_TBL, TRAP_UP_TBL, LINK_AVAIL_TBL,SCHED_TIME_OFF,LINKID_TBL,LINK_T_TBL;
CREATE TEMPORARY TABLE LINKID_TBL(LinkId INTEGER UNSIGNED);
CREATE TEMPORARY TABLE LINK_T_TBL( LinkID INTEGER,
				trapType INTEGER,
				OperStatus VARCHAR(10),
				rcvd_time_1 TIMESTAMP);

INSERT INTO LINK_T_TBL select a.LinkId,a.trapType,a.Operstatus,a.rcvd_time_1 from TRAP_TBL a,TEMP_LINK b where trapType = 2 and a.LinkID = b.LinkID;
...
DO
	...
	select count(*) into @avail from LINK_DAILY_AVAIL_TBL D,TEMP_LINK L
                        WHERE D.LinkID = L.LinkID
                        AND Time = @time;
	if(@avail > 0)
	then 
	INSERT INTO LINK_AVAIL_TBL(LinkID,NoOfOutage,TotalOutage,Availabilty,NWH_Time,SDT_Time,Time) 
		SELECT D.LinkID,NoOfOutage,TotalOutage,Availabilty,NWH_Time,SDT_Time,CONCAT(p_StartTime,"-",p_EndTime) from LINK_DAILY_AVAIL_TBL D,TEMP_LINK L
			WHERE D.LinkID = L.LinkID
			AND Time = @time;
	else 
		SET calc = 1;
	...
	INSERT INTO TRAP_UP_TBL
        ...
        INSERT INTO LINK_AVAIL_TBL(LinkID,Time)
                SELECT LinkID, CONCAT(p_StartTime,"-",p_EndTime)
                        from TEMP_LINK; 
	INSERT INTO LINKID_TBL (select * from TEMP_LINK);
...
DROP TABLE IF EXISTS TEMP_LINK,LINKID_TBL;
