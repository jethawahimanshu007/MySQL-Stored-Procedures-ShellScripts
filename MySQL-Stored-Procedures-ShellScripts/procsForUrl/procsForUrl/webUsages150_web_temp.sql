DROP PROCEDURE IF EXISTS WebUsage_temp; 
DELIMITER |
CREATE PROCEDURE WebUsage_temp(flag INTEGER, StartTime TimeStamp, EndTime TimeStamp)
BEGIN
DECLARE a INTEGER;
...
FROM URL150 a, WEB_PING_STATS_TBL b,NODE_TBL c where b.NodeIp=c.NodeID and b.UrlName like CONCAT('%',a.Url,'%') and b.TimeStamp>StartTime and b.TimeStamp<EndTime ;
...
FROM WEB_STATS_TBL ws, URL150 f
...
FROM URL150 a, WEB_PING_STATS_TBL b,NODE_TBL c where b.UrlName like CONCAT('%',a.Url,'%') and b.TimeStamp>StartTime and b.TimeStamp<EndTime group by a.URL,b.NodeIp,HOUR(TimeStamp);
...
/*SET @fileName=CONCAT(@dir_name,'/URL150_',DATE(startTime),'.csv');
SET @query=CONCAT('SELECT * FROM WEB_USAGE_TBL INTO OUTFILE \'',@fileName,'\' fields terminated by \',\'');
PREPARE stmt1 from @query;
EXECUTE stmt1;
DEALLOCATE PREPARE stmt1;
*/
SELECT * FROM WEB_USAGE_TBL /*into outfile '/tmp/WebUrl150.csv' fields terminated by ','*/ ;
